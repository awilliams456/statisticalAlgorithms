function [w] = predictions()
% This function finds the most likely weights based on the EM algorithm.
% outputs:
%   A: the most likely bayesian network identified by the EM algorithm
%%
% read in the data
fileID = fopen("house-votes-84.data",'r');
formatSpec = '%s';
text = fscanf(fileID,formatSpec);
splitText = split(text, newline);
splitText = splitText(1:300,1);
splitText = split(splitText,",");
samples = transpose(splitText(:,1:3));
%%
n = 3; % n is the number of nodes in the graph
M = size(samples,2); % m is the number of samples
% get the number of potential values
K = 4;
% initialize all weights to ones
w = ones(6,K);
% get array of possible colors
Values = ['republican', 'democrat', 'y', 'n'];
% adjacency matrices for each model
A1 = [0 0 0; 1 0 0; 1 1 0];
A2 = [0 0 1; 1 0 1; 0 0 0];
A3 = [0 1 1; 0 0 1; 0 0 0];
A4 = [0 1 0; 0 0 0; 1 1 0];
A5 = [0 0 0; 1 0 1; 1 0 0];
A6 = [0 1 1; 0 0 0; 0 1 0];
% initialize all nodes
% in this context, neighbors means parents
for i=1:n
    node1(i).label = i;
    node1(i).Neighbors = find(A1(i,:)~=0);
    node2(i).label = i;
    node2(i).Neighbors = find(A2(i,:)~=0);
    node3(i).label = i;
    node3(i).Neighbors = find(A3(i,:)~=0);
    node4(i).label = i;
    node4(i).Neighbors = find(A4(i,:)~=0);
    node5(i).label = i;
    node5(i).Neighbors = find(A5(i,:)~=0);
    node6(i).label = i;
    node6(i).Neighbors = find(A6(i,:)~=0);
end
% get the observed counts for each value per sample
k_counts = zeros(M,K);
for i=1:M
    [~, ~, sample] = find(samples(:,i));
    for k=1:K
        k_counts(i,k) = size(find(sample==Values(k)),1);
    end
end
%%
% for each model
...
while converged == false
    for model = 1:6
        for m = 1:M
            % get q for each m for each model
